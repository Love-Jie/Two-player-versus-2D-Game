	...
	void move_and_collide(int delta)
	{
		...
		if (velocity.y > 0)
		{
			for (const Platform& platform : platform_list)
			{
				...
				{
					float delta_pos_y = velocity.y * delta;   //玩家一帧竖直距离移动的位置
		...
		if (!is_invulnerable)
		{
			for (Bullet* bullet : bullet_list)
			{
				...
				if (bullet->check_collision(position, size))
				{
					...
					if (hp <= 0)
					{
						velocity.x = last_hurt_direction.x < 0 ? 0.25f : -0.35f;
						velocity.y = -1.0f;
					}
				}
			}
...
protected:
	...
	bool is_facing_right = true;   //玩家角色是否朝向右
	...
	bool is_invulnerable = false;			//玩家是否处于无敌状态
